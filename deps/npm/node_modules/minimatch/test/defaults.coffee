...
tap.test "basic tests", (t) ->
  ...
  [
    ...
    return t.comment(c)  if typeof c is "string"
    pattern = c[0]
    expect = c[1].sort(alpha)
    options = c[2]
    f = c[3] or files
    tapOpts = c[4] or {}
    Class = mm.defaults(options).Minimatch
    m = new Class(pattern, {})
    r = m.makeRe()
    tapOpts.re = String(r) or JSON.stringify(r)
    tapOpts.files = JSON.stringify(f)
    tapOpts.pattern = pattern
    tapOpts.set = m.set
    tapOpts.negated = m.negate
    actual = mm.match(f, pattern, options)
    actual.sort alpha
...
tap.test "global leak test", (t) ->
  globalAfter = Object.keys(global)
