...
test "read byte", (t) ->
  reader = new BerReader(new Buffer([0xde]))
  t.ok reader
  t.equal reader.readByte(), 0xde, "wrong value"
  t.end()
  return

test "read 1 byte int", (t) ->
  reader = new BerReader(new Buffer([
    0x02
    0x01
    0x03
  ]))
...
test "read string", (t) ->
  dn = "cn=foo,ou=unit,o=test"
  buf = new Buffer(dn.length + 2)
  buf[0] = 0x04
  buf[1] = Buffer.byteLength(dn)
  buf.write dn, 2
  reader = new BerReader(buf)
  t.ok reader
...
test "long string", (t) ->
  buf = new Buffer(256)
  o = undefined
  s = "2;649;CN=Red Hat CS 71GA Demo,O=Red Hat CS 71GA Demo,C=US;" + "CN=RHCS Agent - admin01,UID=admin01,O=redhat,C=US [1] This is " + "Teena Vradmin's description."
  buf[0] = 0x04
  buf[1] = 0x81
  buf[2] = 0x94
  buf.write s, 3
  ber = new BerReader(buf.slice(0, 3 + s.length))
