...
# 
ruint8 = (buffer, endian, offset) ->
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
  throw (new Error("missing offset"))  if offset is `undefined`
  throw (new Error("Trying to read beyond buffer length"))  if offset >= buffer.length
  buffer[offset]

#
# * For 16 bit unsigned numbers we can do all the casting that we want to do.
# 
rgint16 = (buffer, endian, offset) ->
  val = 0
  if endian is "big"
    val = buffer[offset] << 8
    val |= buffer[offset + 1]
  ...
  val
ruint16 = (buffer, endian, offset) ->
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
  throw (new Error("missing offset"))  if offset is `undefined`
  throw (new Error("Trying to read beyond buffer length"))  if offset + 1 >= buffer.length
  rgint16 buffer, endian, offset
...
rgint32 = (buffer, endian, offset) ->
  ...
  val
ruint32 = (buffer, endian, offset) ->
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
  throw (new Error("missing offset"))  if offset is `undefined`
  throw (new Error("Trying to read beyond buffer length"))  if offset + 3 >= buffer.length
  rgint32 buffer, endian, offset
...
rgint64 = (buffer, endian, offset) ->
  ...
  val
ruint64 = (buffer, endian, offset) ->
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
  throw (new Error("missing offset"))  if offset is `undefined`
  throw (new Error("Trying to read beyond buffer length"))  if offset + 7 >= buffer.length
  rgint64 buffer, endian, offset
...
rsint8 = (buffer, endian, offset) ->
  ...
  return (buffer[offset])  unless neg
  (0xff - buffer[offset] + 1) * -1
...
rfloat = (buffer, endian, offset) ->
  ...
  return (Number.POSITIVE_INFINITY)  if not sign and exponent is maxexp and mantissa is 0
  return (Number.NEGATIVE_INFINITY)  if sign and exponent is maxexp and mantissa is 0
  return (Number.NaN)  if exponent is maxexp and mantissa isnt 0
  ...
  #	 
  return (0)  if exponent is 0 and mantissa is 0
...
# 
prepuint = (value, max) ->
  throw (new (Error("cannot write a non-number as a number")))  unless typeof (value) is "number"
  throw (new Error("specified a negative value for writing an " + "unsigned value"))  if value < 0
  throw (new Error("value is larger than maximum value for " + "type"))  if value > max
  throw (new Error("value has a fractional component"))  if Math.floor(value) isnt value
  value

#
# * 8-bit version, classy. We can ignore endianness which is good.
# 
wuint8 = (value, endian, buffer, offset) ->
  val = undefined
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
  throw (new Error("missing offset"))  if offset is `undefined`
  throw (new Error("Trying to read beyond buffer length"))  if offset >= buffer.length
...
wgint16 = (val, endian, buffer, offset) ->
  ...
  return
wuint16 = (value, endian, buffer, offset) ->
  val = undefined
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
...
wgint32 = (val, endian, buffer, offset) ->
  ...
  return
wuint32 = (value, endian, buffer, offset) ->
  val = undefined
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
...
wgint64 = (value, endian, buffer, offset) ->
  ...
  return
wuint64 = (value, endian, buffer, offset) ->
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("value must be an array"))  unless value instanceof Array
  throw (new Error("value must be an array of length 2"))  unless value.length is 2
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
  throw (new Error("missing offset"))  if offset is `undefined`
  throw (new Error("Trying to read beyond buffer length"))  if offset + 7 >= buffer.length
...
# 
prepsint = (value, max, min) ->
  throw (new (Error("cannot write a non-number as a number")))  unless typeof (value) is "number"
  throw (new Error("value larger than maximum allowed value"))  if value > max
  throw (new Error("value smaller than minimum allowed value"))  if value < min
  throw (new Error("value has a fractional component"))  if Math.floor(value) isnt value
  value
...
# 
wsint8 = (value, endian, buffer, offset) ->
  val = undefined
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
...
# 
wsint16 = (value, endian, buffer, offset) ->
  val = undefined
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
...
# * unsigned code.
# 
wsint32 = (value, endian, buffer, offset) ->
  val = undefined
  throw (new Error("missing value"))  if value is `undefined`
  throw (new Error("missing endian"))  if endian is `undefined`
  throw (new Error("missing buffer"))  if buffer is `undefined`
...
# * need to be equal.
# 
wsint64 = (value, endian, buffer, offset) ->
  vzpos = undefined
  vopos = undefined
  vals = new Array(2)
  throw (new Error("missing value"))  if value is `undefined`
  ...
  #	 * positive or not.
  #	 
  vzpos = (value[0] * Number.POSITIVE_INFINITY) is Number.POSITIVE_INFINITY
  vopos = (value[1] * Number.POSITIVE_INFINITY) is Number.POSITIVE_INFINITY
  
  #
  #	 * If either of these is zero, then we don't actually need this check.
  #	 
  throw (new Error("Both entries in the array must have " + "the same sign"))  if value[0] isnt 0 and value[1] isnt 0 and vzpos isnt vopos
  ...
  if vzpos
    ...
    throw (new Error("value smaller than minimum " + "allowed value"))  if value[0] is -0x80000000 and value[1] isnt 0
  
  # Fix negative numbers 
  if value[0] < 0 or value[1] < 0
    vals[0] = 0xffffffff - Math.abs(value[0])
    vals[1] = 0x100000000 - Math.abs(value[1])
    if vals[1] is 0x100000000
      vals[1] = 0
      vals[0]++
  else
    vals[0] = value[0]
...
fracexp = (value) ->
  Math.floor log2(value)
wfloat = (value, endian, buffer, offset) ->
  sign = undefined
  exponent = undefined
  mantissa = undefined
  ebits = undefined
  ...
  if isNaN(value)
    ...
    exponent = 0xff
    mantissa = 23
  else if value is Number.POSITIVE_INFINITY
    sign = 0
    exponent = 0xff
    mantissa = 0
  else if value is Number.NEGATIVE_INFINITY
    sign = 1
    exponent = 0xff
    mantissa = 0
  else
...
# 
wdouble = (value, endian, buffer, offset) ->
  sign = undefined
  exponent = undefined
  mantissa = undefined
  ebits = undefined
  ...
  if isNaN(value)
    ...
    #		 *
    #		 
    if value <= 2.225073858507201e-308 or ebits <= -1023
      exponent = 0
