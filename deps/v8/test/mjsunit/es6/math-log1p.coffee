...
log1p = (x) ->
  ...
  while i <= 20
    ...
    i++
  sum = 0
  sum += terms.pop()  while terms.length > 0
  sum
assertTrue isNaN(Math.log1p(NaN))
assertTrue isNaN(Math.log1p(->
))
assertTrue isNaN(Math.log1p(toString: ->
  NaN
))
assertTrue isNaN(Math.log1p(valueOf: ->
  "abc"
))
...
# Issue 3481.
assertEquals 6.9756137364252422e-03, Math.log1p(8070450532247929 / Math.pow(2, 60))
