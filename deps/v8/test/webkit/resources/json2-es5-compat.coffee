...
(->
  ...
  str = (key, holder) ->
    ...
    value = value.valueOf()  if value.constructor is String or value.constructor is Number or value.constructor is Boolean  if value and ((typeof value) is "object")
    switch typeof value
      when "string"
        quote value
      when "number"
  ...
  if typeof Date::toJSON isnt "function"
    Date::toJSON = (key) ->
      @getUTCFullYear() + "-" + f(@getUTCMonth() + 1) + "-" + f(@getUTCDate()) + "T" + f(@getUTCHours()) + ":" + f(@getUTCMinutes()) + ":" + f(@getUTCSeconds()) + "Z"
  cx = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g
  escapable = /[\\\"\x00-\x1f]/g
  gap = undefined
  indent = undefined
  ...
  if typeof JSON.parse isnt "function"
    JSON.parse = (text, reviver) ->
      ...
      throw new SyntaxError("JSON.parse")return
  return
)()
